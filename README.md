...
EPRP 是教学协议，不是完整的运行时。宿主 Agent 必须负责：

- 只向模型暴露真实可用的工具和能力；
- 校验 Probe Assessment、Progress 等结构化输出；
- 验证资料、引用、工具调用和持久化是否真的发生；
- 用确定性规则执行状态转移和失败计数；
- 在两次有效失败后触发先修知识修复或阻塞点澄清。

模型可以提出教学建议，但不能自行证明“已经搜索”“已经读取”“已经保存”或“学习者已经掌握”。具体协议见 `references/host-adapter-contract.md`。

### 5. EPRP Lesson Engine
